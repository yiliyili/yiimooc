...
class ProductController extends CommonController
{
    ...
    private function upload()
    {
        ...
        $key = uniqid();
        $qiniu->uploadFile($_FILES['Product']['tmp_name']['cover'], $key);//封面
        $cover = $qiniu->getLink($key);
        $pics = [];
        foreach ($_FILES['Product']['tmp_name']['pics'] as $k => $file) {//多张商品图
    ...
    public function actionMod()
    {
        ...
        if (Yii::$app->request->isPost) {
            ...
            if ($_FILES['Product']['error']['cover'] == 0) {//说明要上传覆盖
                $key = uniqid();
                $qiniu->uploadFile($_FILES['Product']['tmp_name']['cover'], $key);
                $post['Product']['cover'] = $qiniu->getLink($key);//新上传的商品链接
                $qiniu->delete(basename($model->cover));//删除七牛上的老图
